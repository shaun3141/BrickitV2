...
import { listUserCreations, deleteCreation, generateThumbnail } from '@/lib/creationService';
...
export function ProfilePage({ open, onOpenChange, onLoadCreation }: ProfilePageProps) {
  ...
  const handleDeleteCreation = async (creationId: string) => {
    ...
    try {
      const { error } = await deleteCreation(creationId, user.id);
      if (error) throw error;
              ...
              {isLoadingCreations ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {creations.map((creation) => {
                    // Use preview_image_url if available, otherwise generate thumbnail
                    const imageUrl = creation.preview_image_url || generateThumbnail(
                      creation.pixel_data,
                      creation.width,
                      creation.height,
                      4
