...
import { ALL_FILTERS, getFiltersByCategory } from '@/utils/image/filters';
          ...
          {CATEGORIES.map(category => {
            ...
            return (
              <button
                ...
                aria-label={`${category.label} filter category, ${count} ${count === 1 ? 'option' : 'options'}`}
                aria-pressed={isActive}
              >
                <Icon className={`h-4 w-4 flex-shrink-0 ${isActive ? 'text-primary' : 'text-muted-foreground'}`} aria-hidden="true" />
